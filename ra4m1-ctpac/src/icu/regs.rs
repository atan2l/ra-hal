...
impl Ielsr {
    ...
    #[must_use]
    #[inline(always)]
    pub const fn iels(&self) -> u8 {
        let val = (self.0 >> 0usize) & 0xff;
        val as u8
    }
    #[doc = "ICU Event selection to NVIC Set the number for the event signal to be linked ."]
    #[inline(always)]
    pub const fn set_iels(&mut self, val: u8) {
...
impl defmt::Format for Ielsr {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "Ielsr {{ iels: {=u8:?}, reserved: {=u8:?}, ir: {=bool:?}, reserved_2: {=u8:?}, dtce: {=bool:?}, reserved_3: {=u8:?} }}",
            self.iels(),
            self.reserved(),
            self.ir(),
            self.reserved_2(),
...
impl Selsr0 {
    ...
    #[must_use]
    #[inline(always)]
    pub const fn sels(&self) -> u8 {
        let val = (self.0 >> 0usize) & 0xff;
        val as u8
    }
    #[doc = "SYS Event Link Select"]
    #[inline(always)]
    pub const fn set_sels(&mut self, val: u8) {

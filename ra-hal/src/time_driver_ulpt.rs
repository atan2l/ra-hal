...
//!
//! Currently hardcoded to take up `ULPT1`.
//!
//! # Notes
...
impl TimerPause for TimerPeripheral {
    ...
    where
        F: FnOnce(&Self),
    {
        self.ulptcr().modify(|r| r.set_tstop(true));
        while self.ulptcr().read().tcstf() {}
        ...
        self.ulptcr().modify(|r| r.set_tstart(true));
        while !self.ulptcr().read().tcstf() {}
    }
}

trait Instance: crate::timer_ulpt::Instance<Self::Width> + Send + Sync + 'static {
...
cfg_select! {
    ulpt => {
        use crate::peripherals::ULPT1;
        driver_instance!(ULPT1, u32);
        type TimerPeripheral = crate::pac::ulpt::Ulpt;
        const PERIOD : u32 = u32::MAX;
        const WIDTH : usize = 32;
    }
}

struct UlptDriver<I: Instance> {
...
impl<I: Instance> UlptDriver<I> {
    ...
    pub(crate) fn init(&'static self) {
        ...
        let ulpt_source = match clock_config.sosc {
            true => {
        ...
        timer.ulptcr().write_value(Ulptcr(0));
        while timer.ulptcr().read().tcstf() {}
        ...
        timer.ulptmr2().modify(|r| r.set_cks(Cks::Div1));
        for _ in 0..2 {
            timer.ulpt().read();
        }

        timer.ulptmr1().modify(|r| {
            r.set_tck(ulpt_source);
            r.set_tmod(Tmod::Timer);
        });
        timer.ulptmr2().modify(|r| r.set_cks(Cks::Div1));
        timer.ulpt().write_value(PERIOD);
        timer.ulptcr().modify(|r| {
            r.set_tundf(false);
            r.set_tcmaf(false);
            r.set_tcmbf(false);
        });
    ...
    fn interrupted_underflow(&'static self) {
        critical_section::with(|_cs| {
            ...
            timer.ulptcmsr().modify(|r| r.set_tcmea(true));
        });
    }
    ...
    fn set_alarm(&self, cs: &CriticalSection, timestamp: u64) -> bool {
        ...
        if diff < u64::from(PERIOD) {
            ...
            timer.while_paused(|timer| {
                // Guess there's some sort of waiting period after updating ulptcr too.
                // TODO: Looks like 1 or 2 AGT[SL]CLK cycles is appropriate.
                for _ in 0..500 {
                    timer.ulptcr().read();
                }

                timer.ulpt().write_value(cur);

                timer.ulptcma().write_value(desired as _);
            });
...
impl<I: Instance> Driver for UlptDriver<I> {
    fn now(&self) -> u64 {
        let timer = I::regs();
...
embassy_time_driver::time_driver_impl!(static DRIVER: UlptDriver<ULPT1> = UlptDriver {

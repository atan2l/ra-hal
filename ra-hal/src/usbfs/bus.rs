...
use crate::pac::usbfs::regs::{Cfifosel, Dcpctr, Dfifosel};
...
impl<'d, I: Instance + 'static> UsbBus for Bus<'d, I> {
    ...
    fn reset(&self) {
        ...
        let mut d0fifosel = Dfifosel::default();
        d0fifosel.set_mbw(true);
        regs.dfifosel(0).write_value(d0fifosel);
        let mut d1fifosel = Dfifosel::default();
        d1fifosel.set_mbw(false);
        regs.dfifosel(1).write_value(d1fifosel);
    ...
    fn write(&self, ep_addr: EndpointAddress, buf: &[u8]) -> UsbResult<usize> {
        ...
        if state.ep0_last_setup_dir_out && buf.is_empty() {
            ...
            regs::update_bempenb(regs, &mut state.bempenb_shadow, 0, 1u16);
            regs::set_dcp_pid(regs, DcpctrPid::Buffer, true);
            state.ep0_last_setup_dir_out = false;
            return Ok(0);
        }
        ...
        let mut state = self.state.borrow_mut();
        regs::set_dcp_pid(regs, DcpctrPid::Buffer, false);
        regs::clear_bemp0(regs);
    ...
    fn set_stalled(&self, ep_addr: EndpointAddress, stalled: bool) {
        if ep_addr.index() == 0 {
            ...
            dcpctr.set_sqclr(!stalled && next_pid != DcpctrPid::Buffer);
            regs.dcpctr().write_value(dcpctr);

            let mut state = self.state.borrow_mut();
            if !stalled && next_pid == DcpctrPid::Buffer && live_ctsq == Ctsq::_001 {
    ...
    fn poll(&self) -> PollResult {
        ...
        if latched.intsts0.dvst() {
            ...
            if latched.intsts0.dvsq() == Dvsq::Default {

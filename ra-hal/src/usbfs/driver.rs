...
use crate::{
    ...
    gpio::{Flex, PortFunction, WithOpenDrain},
    interrupt::{
        Interrupt,
    ...
    pac::{
        self,
        usbfs::{
            regs::{Dcpctr, Intenb1, Intsts0, Intsts1},
            vals::DcpctrPid,
        },
    },
    peripherals,
...
pub(super) trait SealedInstance: ModuleStop {
...
pub struct Driver<'d, I: Instance> {
    ...
    _config: Config,
    _vbus: Flex<'d, WithOpenDrain>,
    _dm: Flex<'d, WithOpenDrain>,
    _dp: Flex<'d, WithOpenDrain>,
}
...
impl<'d, I: Instance + 'static> Driver<'d, I> {
    ...
    >(
        ...
        dm_pin.set_as_pf(PortFunction::HiZ);
        dp_pin.set_as_pf(PortFunction::HiZ);
...
fn init_usbfs_registers(regs: pac::usbfs::Usbfs) {
    ...
    regs.dfifosel(0).write(|w| w.set_mbw(true));
    regs.dfifosel(1).write(|w| w.set_mbw(false));
    regs.intsts1().write_value(Intsts1(0));
    regs.intsts0().write_value(Intsts0(0));
    ...
    regs.intenb1().write_value(Intenb1(0));
}
...
impl SealedInstance for peripherals::USBFS {
    const INTERRUPT_EVENT: InterruptEvent = InterruptEvent::UsbfsInt;

...
use crate::pac::usbfs::vals::{CfifoselCurpipe, DcpctrPid, DfifoselCurpipe, PipectrPid, Type};
...
pub(super) fn select_d0fifo(regs: pac::usbfs::Usbfs, pipe: u8) {
    let mut expected = Dfifosel::default();
    expected.set_curpipe(DfifoselCurpipe::from_bits(pipe));
    expected.set_mbw(true);
    regs.dfifosel(0).write_value(expected);
    for _ in 0..CFIFO_READY_SPINS {
        let fifosel = regs.dfifosel(0).read();
...
pub(super) fn write_d0fifo(regs: pac::usbfs::Usbfs, buf: &[u8]) {
    ...
    let fifosel = regs.dfifosel(0).read().0;
    let mut d0fifosel = Dfifosel(fifosel);
    d0fifosel.set_mbw(false);
    regs.dfifosel(0).write_value(d0fifosel);

    let ptr8 = regs.dfifo(0).as_ptr() as *mut u8;
    for &byte in buf {
...
pub(super) fn clear_d0fifo_buffer(regs: pac::usbfs::Usbfs) {
    let mut ctr = Dfifoctr::default();
    ctr.set_bclr(true);
    regs.dfifoctr(0).write_value(ctr);
}
...
pub(super) fn pipe_busy(pipe: u8) -> bool {
    let regs = usbfs();
    match pipe {
        1..=9 => regs.pipectr((pipe - 1) as usize).read().pbusy(),
        _ => panic!("Invalid pipe, supported are 1..=9"),
    }
}
    ...
    match pipe {
        1..=9 => {
            ...
            ctr.set_pid(match pid {
                PipePid::Nak => PipectrPid::Nak,
                PipePid::Buf => PipectrPid::Buffer,
                PipePid::Stall => PipectrPid::Stall2,
            });
...
pub(super) fn pipe_pid_is_buf(pipe: u8) -> bool {
    let regs = usbfs();
    match pipe {
        1..=9 => regs.pipectr((pipe - 1) as usize).read().pid() == PipectrPid::Buffer,
        _ => panic!("Invalid pipe, supported are 1..=9"),
    }
}
...
pub(super) fn set_pipe_nak_wait(pipe: u8) {
    ...
    assert!(
        (1_u8..=9).contains(&pipe),
        "Invalid pipe, supported are 1..=9"
    );

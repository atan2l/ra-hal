...
fn generate_peripherals(metadata: &Metadata, package: String) -> anyhow::Result<()> {
    ...
    for peripheral in peripheral_list.iter() {
        ...
        match (
            ...
            ("USBFS", _driver) => {
                for signal in peripheral.signals.iter() {
                    let signal_ident = match *signal {
                        "USBFS_VBUS" => format_ident!("usbfs_vbus_pin"),
                        "USBFS_DM" => format_ident!("usbfs_dm_pin"),
                        "USBFS_DP" => format_ident!("usbfs_dp_pin"),
                        _ => continue,
                    };
                    ...
                    for pin_config in pins {
                        ...
                        let pfunc = format_ident!(
                            ...
                            pin_config
                                .pfunc
                                // The 4L1 doesn't specify a peripheral function.
                                // Jokes on Renesas, the 4M1 does *but refuses to honor the USB_FS pfunc*. smdh.
                                .unwrap_or("USB_FS")
                                .to_string()
                                .to_case(Case::Pascal)
                        );
                        ...
                        contents.extend(quote! {
                            crate::usbfs::#signal_ident!(#peripheral_ident, #pin_ident, #pfunc);
                        });
                    }
                }
            }
